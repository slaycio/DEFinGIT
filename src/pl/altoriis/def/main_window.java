...
public class main_window {
	...
	public static void draw_center() {
		...
		String[][] lista = db_con.get_data(slowniki[cmb1.getSelectionIndex()][2][0], Integer.parseInt(slowniki[cmb1.getSelectionIndex()][1][0]));
			...
			// ten listener powduje bledy	
				new_dict_data.addListener (SWT.MouseDown, new Listener () {
					public void handleEvent (Event event) {
						...
						/*Boolean checkTables[] = new Boolean[new_dict_data.getColumnCount()];
						
						for (int i=0; i<dict_data.getColumnCount(); i++) {
						
						checkTables[i]	= dict_data.getItem(dict_data.getSelectionIndex()).getText(i) == new_dict_data.getItem(new_dict_data.getSelectionIndex()).getText(i);
						...
						if (Arrays.asList(checkTables).contains(false))
						{
							...
							lblNewLabel_1.setText(dict_data.getItem(dict_data.getSelectionIndex()).getText(0));  // label do wyswietlenie ze nacislem 
							lblNewLabel_2.setText(new_dict_data.getItem(new_dict_data.getSelectionIndex()).getText(0));  // label do wyswietlenie ze nacislem 
			...
			} // tu sie konczy wewnetrzny selection event -- przed tym trzeba wstawic kod by by³ uruchomiony po zmianie w comboboxie
...
/////////////////  TUTAJ WSTAWIAJ	
	    ...
	    shell.addListener (SWT.Resize,  new Listener () {
	        public void handleEvent (Event e) {
	         ...
	         // draw_center().dispose();
	        // draw_center();
	          //dict_data_comp.setSize(act_size.x-200,600);
	          //dict_data_comp.redraw();
	        }
	      });
